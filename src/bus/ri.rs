...
impl RdRegs {
    pub fn read_reg(&self, addr: u32) -> IoResult<u32> {
        Ok(match addr {
            ...
            RDRAM_REG_DELAY        => self.reg_delay,
            RDRAM_REG_MODE         => self.reg_mode,
            RDRAM_REG_REF_INTERVAL => self.reg_ref_interval,
            RDRAM_REG_REF_ROW      => self.reg_ref_row,
            RDRAM_REG_RAS_INTERVAL => self.reg_ras_interval,
            RDRAM_REG_MIN_INTERVAL => self.reg_min_interval,
            RDRAM_REG_ADDR_SELECT  => self.reg_addr_select,
            RDRAM_REG_DEVICE_MANUF => self.reg_device_manuf,
            _ => return Err("Unsupported RDRAM register"),
        })
    }
    ...
    pub fn write_reg(&mut self, addr: u32, word: u32) -> IoResult<()> {
        Ok(match addr {
            ...
            RDRAM_REG_DELAY        => self.reg_delay = word,
            RDRAM_REG_MODE         => self.reg_mode = word,
            RDRAM_REG_REF_INTERVAL => self.reg_ref_interval = word,
            RDRAM_REG_REF_ROW      => self.reg_ref_row = word,
            RDRAM_REG_RAS_INTERVAL => self.reg_ras_interval = word,
            RDRAM_REG_MIN_INTERVAL => self.reg_min_interval = word,
            RDRAM_REG_ADDR_SELECT  => self.reg_addr_select = word,
            _ => return Err("Unsupported RDRAM register"),
        })
    }
}
